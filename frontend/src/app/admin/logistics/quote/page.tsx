...
export default function QuotePage() {
  ...
  const handleQuote = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch(`http://127.0.0.1:8000/api/v1/shipping/quote?origin_provider=${origin.provider}&origin_city=${origin.city}`, {
        method: 'POST',
    ...
    <div style={{ width: '100%' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '24px' }}>
        ...
        <div className="glass-panel" style={{ padding: '24px' }}>
          <form onSubmit={handleQuote} style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
              ...
              <div className="grid-cols-2">
                <div>
                  ...
                  <select 
                    ...
                    required
                    style={{ width: '100%', padding: '10px 14px', borderRadius: '8px', background: 'var(--input-bg)', border: '1px solid var(--input-border)', color: 'var(--input-text)', appearance: 'none' }}
                  >
            ...
            <div style={{ background: 'rgba(255, 255, 255, 0.02)', padding: '20px', borderRadius: '12px', border: '1px solid var(--card-border)' }}>
              ...
              <div className="grid-cols-4">
                <div>
                  ...
                  <input type="number" min="0.1" step="0.1" value={parcel.weight} onChange={e => setParcel({...parcel, weight: Number(e.target.value)})} required style={{ width: '100%', padding: '10px 14px', borderRadius: '8px', background: 'var(--input-bg)', border: '1px solid var(--input-border)', color: 'var(--input-text)' }} />
                </div>
                <div>
                  <label style={{ display: 'block', marginBottom: '8px', fontSize: '0.9rem', color: 'var(--text-muted)' }}>Largo (cm)</label>
                  <input type="number" min="1" value={parcel.length} onChange={e => setParcel({...parcel, length: Number(e.target.value)})} required style={{ width: '100%', padding: '10px 14px', borderRadius: '8px', background: 'var(--input-bg)', border: '1px solid var(--input-border)', color: 'var(--input-text)' }} />
                </div>
                <div>
                  <label style={{ display: 'block', marginBottom: '8px', fontSize: '0.9rem', color: 'var(--text-muted)' }}>Ancho (cm)</label>
                  <input type="number" min="1" value={parcel.width} onChange={e => setParcel({...parcel, width: Number(e.target.value)})} required style={{ width: '100%', padding: '10px 14px', borderRadius: '8px', background: 'var(--input-bg)', border: '1px solid var(--input-border)', color: 'var(--input-text)' }} />
                </div>
                <div>
                  <label style={{ display: 'block', marginBottom: '8px', fontSize: '0.9rem', color: 'var(--text-muted)' }}>Alto (cm)</label>
                  <input type="number" min="1" value={parcel.height} onChange={e => setParcel({...parcel, height: Number(e.target.value)})} required style={{ width: '100%', padding: '10px 14px', borderRadius: '8px', background: 'var(--input-bg)', border: '1px solid var(--input-border)', color: 'var(--input-text)' }} />
                </div>
              </div>
            </div>

            <button type="submit" className="btn-primary" style={{ width: '100%', padding: '14px', fontSize: '1.1rem', display: 'flex', justifyContent: 'center', alignItems: 'center', gap: '8px' }} disabled={loading}>
        ...
        <div className="glass-panel" style={{ padding: '24px' }}>
          <div style={{ position: 'sticky', top: '2rem' }}>
            <h3 style={{ fontSize: '1.25rem', marginBottom: '16px', color: 'var(--foreground)', borderBottom: '1px solid var(--card-border)', paddingBottom: '16px' }}>
              Tarifas Disponibles
            </h3>
