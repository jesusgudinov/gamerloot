    ...
    async def parse_and_sync(self, db: AsyncSession, exchange_service=None, log_func=None):
        ...
        try:
            ...
            for i in range(min(20, len(df_raw))):
                ...
                matches = sum(1 for req in ["SKU", "VOLUMEN", "INVENTARIO"] if req in row_vals)
                if matches >= 2:
                    header_idx = i
                    break
        ...
        for index, row in df.iterrows():
            ...
            if pd.notna(raw_cond):
                condicion = str(raw_cond).strip().upper()
                if condicion and condicion not in ["NUEVO", "NUEVA", "NEW", "NA"]:
                    continue
