...
@router.post("/mienvio")
async def mienvio_webhook(request: Request, db: AsyncSession = Depends(get_db)):
    """
    Recibe eventos de Mienvio.
    """
    ...
    if event_type and "shipment" in event_type:
        attrs = data.get("attributes", {})
        reference = attrs.get("reference", "")
        
        # Reference format: Order-123-06700
        if reference.startswith("Order-") or reference.startswith("LOOT-"):
            # Support both Order-123-06700 and LOOT-123-06700 reference formats
            parts = reference.split("-")
            if len(parts) >= 2:
                try:
                    # In Mienvio we send order_id=LOOT-123-45403, we need to extract the ID properly
                    # Wait, our order reference from fulfillment is `f"{order.id}-{wh['zip']}"` e.g. "27-02300"
                    order_id = int(parts[0])
                    origin_zip = parts[1] if len(parts) > 1 else None
                    ...
                    if order and order.shipments_data:
                        ...
                        if updated:
                            # Verify if all shipments are delivered to mark main order as delivered
                            all_delivered = all(s.get("status") == "Entregado" for s in shipments)
                            
                            update_stmt = update(Order).where(Order.id == order_id).values(
                    ...
                    print(f"Error procesando webhook de Mienvio: {e}")
