...
async def run():
    ...
    async with engine.connect() as conn:
        ...
        missing_attrs = [
            ...
            ("Soporte de radiador", "soporte-de-radiador")
        ]
        
        for attr, slug in missing_attrs:
            res = await conn.execute(text("SELECT id FROM product_attributes WHERE name = :n"), {"n": attr})
            if not res.fetchone():
                await conn.execute(text("INSERT INTO product_attributes (name, slug, is_filterable, is_for_configurator, is_connector) VALUES (:n, :s, true, true, false)"), {"n": attr, "s": slug})
